...
def test_autostructure_coupling():
    ...
    # Test 1: No coupling specified (should default to LS)
    print("\n[Test 1] AUTOSTRUCTURE with coupling=None (should default to LS)")
    calc1 = AtomicCalculation(
        name="test_as_default",
        element="C",
        charge=2,
        calculation_type="structure",
        coupling=None,  # Should default to LS
        configurations=["1s2.2s2"],
        output_dir="outputs/test_coupling",
        code="autostructure",
    )
    print(f"  Coupling after init: {calc1.coupling}")
    assert calc1.coupling == "LS", f"Expected LS, got {calc1.coupling}"
    print("  ✓ Defaults to LS")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("=" * 80)
        print("\nSummary:")
        print("  • AUTOSTRUCTURE: Defaults to LS when coupling=None")
