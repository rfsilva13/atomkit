...
"""
    ...
    def close(self):
        ...
        if self._file is None:
            # Open the file if not already open (for non-context manager usage)
            self._file = open(self.filename, "w")
            self._write_header()
    ...
    def OptimizeRadial(self, groups: List[str], **kwargs):
        ...
        """
        groups = self._unique_preserve_order(groups)
    ...
    def TransitionTable(
        ...
        **kwargs,
        ...
        """
        lower_groups = self._unique_preserve_order(lower_groups)
        upper_groups = self._unique_preserve_order(upper_groups)
        # Always include the multipole argument explicitly. FAC uses 0 to mean
        # "all multipoles/default", but some workflows (and users) expect an
        # explicit 0 in the SF file. Emit the integer value provided so callers
        # can control whether 0 or another code is written.
        args = [output_file, lower_groups, upper_groups, multipole]
    ...
    def CITable(
        ...
        **kwargs,
        ...
        """
        bound_groups = self._unique_preserve_order(bound_groups)
        free_groups = self._unique_preserve_order(free_groups)
    ...
    def _unique_preserve_order(self, seq: Optional[List[str]]) -> List[str]:
        """Return a list with duplicates removed, preserving first occurrence order.

        If `seq` is None, return None. If `seq` is not a list, return it unchanged.
        """
        if not seq:
            return []
        seen = set()
