...
def _build_shell_label_map(max_n: int = 7) -> dict[str, str]:
    """
    Generates the mapping from shell structure strings to Siegbahn notation labels.

    Siegbahn notation is the standard X-ray spectroscopy labeling system using
    symbols like K, L1, L2, L3, M1-M5, etc. to denote electronic shells based
    on their quantum numbers (n, l, j).

    Args:
        max_n: The maximum principal quantum number to generate labels for.

    Returns:
        A dictionary mapping shell strings (e.g., "1s", "2p-") to Siegbahn labels
        (e.g., "K", "L2").
    """
...
"""
Mapping from simple shell structure string (e.g., '1s', '2p-') to Siegbahn notation label.

Siegbahn notation is the standard X-ray spectroscopy labeling system:
- K shell: 1s
- L shells: L1 (2s), L2 (2p-), L3 (2p+)
- M shells: M1 (3s), M2 (3p-), M3 (3p+), M4 (3d-), M5 (3d+)
- N, O, P, Q shells follow the same pattern for higher principal quantum numbers
