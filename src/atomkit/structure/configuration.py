...
# Import constants needed from parent package
from ..constants import SHELL_LABEL_MAP  # Import the X-ray label map
from ..constants import (ANGULAR_MOMENTUM_MAP, L_SYMBOLS,
                         get_max_shell_occupation)
# Import Shell from same directory
...
class Configuration:
    ...
    def copy(self) -> "Configuration":
        ...
        # Create new configuration without going through __init__ to preserve zero-occupation shells
        new_config = Configuration()
        # Directly copy _shells dict to preserve all shells including zero-occupation ones
        new_config._shells = {
            key: Shell(s.n, s.l_quantum, s.occupation, s.j_quantum)
            for key, s in self._shells.items()
        }
        return new_config
    ...
    def to_string(self, separator: str = ".") -> str:
        ...
        # Include all shells, even those with 0 occupation (physically meaningful for holes)
        shell_strings = [str(shell) for shell in self.shells]
    ...
    def from_string(cls, config_string: str) -> "Configuration":
        ...
        for part in parts:
            try:
                ...
                parsed = Shell.from_string(part)
                if isinstance(parsed, list):
                    shells.extend(parsed)
                else:
        ...
        for i in range(start_shell_index, len(shells_list)):
            ...
            if shell.occupation > 0:
                ...
                try:
                    ...
                    # For hole configurations, keep shells even with zero occupation
                    # They represent physically meaningful vacancies (e.g., K-shell holes)
                    if removed_shell.occupation == 0:
                        # Manually add the zero-occupation shell by directly modifying _shells
                        key = (removed_shell.n, removed_shell.l_quantum, removed_shell.j_quantum)
                        next_config._shells[key] = removed_shell
                    else:
                        next_config.add_shell(removed_shell, combine_occupation=False)
                except ValueError:
    ...
    def generate_hole_configurations(self, num_holes: int = 1, group: bool = False) -> list["Configuration"]:
        """
        ...
        results = sorted(list(config_set), key=lambda c: str(c))

        # If requested, annotate each generated configuration with a simple
        # group label (same label for all generated configs). This allows
        # downstream code (e.g., SF postprocessing) to choose non-numbered
        # group names such as 'one_hole' or 'two_hole'. We attach a
        # `group_label` attribute to the Configuration objects.
        if group:
            if num_holes == 1:
                base_label = "one_hole"
            elif num_holes == 2:
                ...
                base_label = f"{num_holes}_hole"

            for cfg in results:
                try:
                    setattr(cfg, "group_label", base_label)
                except Exception:
                    # Be defensive: if assignment fails for any reason,
                    # continue without breaking generation.
                    pass
